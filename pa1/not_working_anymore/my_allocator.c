...
extern Addr my_malloc(unsigned int _length) {
  ...
  if (free_list[size_index] != NULL) {//block_av[size_index] > 0) {
	  //printf("removing from free list\n");
	  //print_allocator();
	char* new_address = remove_from_free_list(size_index, free_list[size_index]);
	if (new_address == NULL)
		return NULL;
	else 
		return new_address + sizeof(header);  
  } 
  else {
	//if there are no available block sizes of the needed type, check if there are any bigger block sizes that can be split
	char* new_address = find_available_block(size_index) + sizeof(header);
	if (new_address == NULL)
		return NULL;
	else 
		return new_address + sizeof(header);  
...
char* remove_from_free_list(int size_index, header* b) {
	...
	header* temp = free_list[size_index];
	if (temp == b && temp != NULL) {//the block is at the front
		free_list[size_index] = temp->next_blk;
	}
	else {
		while (temp != NULL && temp->next_blk != b) {
			temp = temp->next_blk;
		}
		if (temp != NULL)
...
extern int my_free(Addr _a) {
	...
	if (_a == NULL){
		printf("Memory to deallocate was NULL!\n");
		return NULL;
	}
