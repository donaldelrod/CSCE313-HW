...
void process_newthread(RequestChannel & _channel, const std::string & _request) {
	std::cout << "dataserver started new thread" << std::endl;
  	int error;
	...
	try {
		RequestChannel * data_channel = new RequestChannel(new_channel_name, RequestChannel::SERVER_SIDE, nthreads+1);
		nthreads++;
...
int main(int argc, char * argv[]) {
	...
	RequestChannel control_channel("control", RequestChannel::SERVER_SIDE, 256);
	//  std::cout << "done.\n" << std::flush;
