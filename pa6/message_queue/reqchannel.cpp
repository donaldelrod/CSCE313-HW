...
{	
	...
	mqid = mq_open(my_name.c_str(), O_CREAT | O_RDWR, 0644, NULL);
		
	/*
	if (my_side == RequestChannel::SERVER_SIDE) { 
		if (my_name.compare("serv_send") == 0)
			mqid = mq_open(my_name.c_str(), O_CREAT | O_WRONLY, 0644, &attributes);
		else 
			mqid = mq_open(my_name.c_str(), O_CREAT | O_RDONLY, 0644, &attributes);
	}
	else {
		if (my_name.compare("serv_send") == 0)
			mqid = mq_open(my_name.c_str(), O_CREAT | O_RDONLY, 0644, &attributes);
...
std::string RequestChannel::cread() {
	...
	bytes_read = mq_receive(mqid, buf, 8192, NULL);
	
	while (bytes_read < 0) {
		std::cout << "on the " << side_name << " side, ";
		perror("error using cread() ");
		usleep(50000);
	}
...
int RequestChannel::cwrite(std::string _msg) {
	...
	//std::cout << my_name << ":" << side_name << ": writing [" << _msg << "]" << std::endl;
	//std::cout << "msg size: " << sizeof(_msg.c_str()) << std::endl;
	int retval = mq_send(mqid, _msg.c_str(), 8192, NULL);
	
	
	if (retval < 0)
		perror("error using cwrite(): ");
